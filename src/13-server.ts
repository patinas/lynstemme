...
type Env = {
  AI: Ai;
  LynStemmeAgent: DurableObjectNamespace;
  GROQ_API_KEY?: string;
  GROQ_CHAT_MODEL?: string;
  AI_BACKEND?: "auto" | "groq" | "workers-ai" | "local";
...
function history(context: VoiceTurnContext, transcript: string) {
  return [
...
async function openAICompatible(env: Env, transcript: string, context: VoiceTurnContext, local = false) {
  ...
  const response = await fetch(`${base.replace(/\/$/, "")}/chat/completions`, {
    ...
    body: JSON.stringify({ model, messages: history(context, transcript), temperature: 0.3 }),
...
export class LynStemmeAgent extends VoiceAgent<Env> {
  ...
  async onTurn(transcript: string, context: VoiceTurnContext) {
    ...
    if (backend === "groq" || (backend === "auto" && this.env.GROQ_API_KEY)) {
      return openAICompatible(this.env, transcript, context);
    }
    const result = await this.env.AI.run("@cf/meta/llama-3.1-8b-instruct", {
      messages: history(context, transcript), max_tokens: 180
    }) as { response?: string };
    return result.response || "Jeg kunne ikke danne et svar.";
  }

  async onCallStart(connection: Connection) {
    await this.speak(connection, "Hej, du taler med LynStemme. Hvad kan jeg hjælpe med?");
  }
}
...
export default {
  async fetch(request: Request, env: Env) {
    ...
    if (url.pathname === "/health") {
      const backend = env.AI_BACKEND || (env.GROQ_API_KEY ? "groq" : "workers-ai");
      return Response.json({ status: "ok", backend, voice: "cloudflare" });
    }
