...
export default { async fetch(request: Request, env: Env) {
  ...
  if (url.pathname === "/tts-test") { const audio = await new DanishTTS(env.AI).synthesize(url.searchParams.get("text") || "Hej, mit navn er LynStemme. Jeg taler flydende dansk hver dag."); if (!audio) return Response.json({ ok: false }, { status: 502 }); return new Response(audio, { headers: { "content-type": "audio/mpeg" } }); }
