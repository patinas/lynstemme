...
export default { async fetch(request: Request, env: Env) {
  ...
  if (url.pathname === "/stt-check") { const response = await (env.AI as any).run("@cf/deepgram/nova-3", { encoding: "linear16", sample_rate: "16000", language: "da", interim_results: "true", vad_events: "true", endpointing: "400", utterance_end_ms: "1000", smart_format: "true", punctuate: "true" }, { websocket: true }) as Response; const ws = response.webSocket; if (!ws) return Response.json({ ok: false, status: response.status }, { status: 502 }); ws.accept(); ws.close(); return Response.json({ ok: true, model: "nova-3", language: "da", transport: "websocket" }); }
