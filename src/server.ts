...
export default { async fetch(request: Request, env: Env) {
  ...
  if (url.pathname === "/tts-test") { const text = url.searchParams.get("text") || "Hej, mit navn er LynStemme. Jeg taler flydende dansk hver dag."; const result = await (env.AI as any).run("inworld/tts-2-flash", { text, voice_id: "Sophie", language: "da-DK", output_format: "mp3", timestamp_type: "none" }, { returnRawResponse: true }) as unknown as AiResponse; if (result instanceof Response) { if (!result.ok) { const body = await result.text().catch(() => ""); return Response.json({ ok: false, status: result.status, error: body.slice(0, 500) }, { status: 502 }); } return new Response(await result.arrayBuffer(), { headers: { "content-type": "audio/mpeg" } }); } const encoded = result.audio || result.data; if (!encoded) return Response.json({ ok: false, shape: Object.keys(result as object) }, { status: 502 }); return new Response(base64Bytes(encoded), { headers: { "content-type": "audio/mpeg" } }); }
